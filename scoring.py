...
#blood_req=input("ENTER BLOOD REQUESTED  : ")
#if blood_req in blood_rarity:
    #    pass
#else:
   #  print()
#notes=input("ENTER URGENCY TYPE : ")

#if notes in clinical_urgency:
       # pass
#else:
    #  print()
...
def priority_score(blood_req,notes,created_at =None):
    reason=[]
    b=blood_rarity.get(blood_req,0) # ,0 ensures that the output is not none type
    ...
    return {
        "score": s,
        "reason": " | ".join(reason),"TOTAL_SCORE":f"{s}"
    }
    
#test case 
# two_hrs_ago = datetime.datetime.now() - datetime.timedelta(hours=2)
# result = priority_score("O-", "Patient admitted in critical condition in ICU", created_at=two_hrs_ago)

# print("Result for Django:")
# print("Numeric Score:", result["score"])  # Sv sorts using this
# print("Reason Text:", result["reason"])    # Sv displays this in UI
